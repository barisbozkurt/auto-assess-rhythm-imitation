...
import os
import pickle
from data_processing.datapreprocess_rhythm import Recording
import numpy as np
from os import path
...
def save_all_feats(rec, file_path_init):
    ...
    [S, n_frm] = compute_spectrogram(wav_file_path)

    onsets_base_binary = get_binary_labels(onsets_base, n_frm)
    ...
    if path.exists(in_lab_folder + '/' + rec.file_path.replace('.m4a', '.os.txt')):
        onset_labels = read_label(in_lab_folder + '/' + rec.file_path.replace('.m4a', '.os.txt'))
    else:
    ...
    onset_labels_binary = get_binary_labels(onset_labels, n_frm)
    out_file_path = '{}_{}'.format(file_path_init, 'onset_lab_bin.txt')
    np.savetxt(out_file_path, onset_labels_binary.astype(float), fmt='%.1f')

    return onsets_base_binary, onset_labels_binary, n_frm
...
if __name__ == "__main__":
    ...
    test_file_list_path = '../../data/listperformances_test.txt'
    temp_data = np.loadtxt(test_file_list_path, dtype={'names': ('file', 'grade'), 'formats': ('S30', 'i4')})
