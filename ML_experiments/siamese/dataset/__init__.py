...
class Dataset():
    ...
    def __getitem__(self, index):
        if '.npy' in self.datasetid_to_filepath[index]:
            ...
            if method == 'scipy':
                f, t, S = signal.spectrogram(x, srate, nfft=config.nfft, nperseg=config.window_size, noverlap=config.skip_size)
                S = np.transpose(preprocessing.sequence.pad_sequences(10 * np.log(S +1.e-10),maxlen=self.max_len, padding='post'))
                instance = (S - np.mean(S, axis=0)) / np.std(S, axis=0)
            elif method == 'librosa':
                S = librosa.core.stft(y=x, n_fft=2048, win_length=int(config.w_size * srate), hop_length=int(config.s_size * srate))
                S = preprocessing.sequence.pad_sequences(np.abs(S) ** 2, maxlen=config.max_frm, padding='post')
                mel_basis = librosa.filters.mel(sr=srate, n_fft=2048, n_mels=config.num_mels)
    ...
    def index_set(self, audio_path):
        ...
        for root, folders, files in os.walk(audio_path):
            ...
            for f in files:
                ...
                if not (f.endswith(config.file_ext)):
                    continue

                file_path = os.path.join(root, f).replace('\\', '/')
                parts = f.replace(config.file_ext, '').split('_')
                ...
                onset_file_path = file_path.replace(config.file_ext, config.lab_ext)
